...
class HCR_LC_Net(nn.Module):

    def __init__(self, in_channels=15, n_classes=10, feature_scale=2, is_deconv=True, is_batchnorm=True):
        super(HCR_LC_Net, self).__init__()
    ...
    def forward(self, inputs):
        ...
        return decloud, logits
        
        
    def get_last_shared_layer(self):
        return self.center
    def get_last_layer_decloud(self):
        return self.up_concat1
...
if __name__ == '__main__':
    ...
    model = HCR_LC_Net().cuda()
    param = count_param(model)
    y = model(x)
    print('Output shape:', y.shape)
    print('HCR_LC_Net totoal parameters: %.2fM (%d)' % (param / 1e6, param))