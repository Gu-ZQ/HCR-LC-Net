...
from sklearn.metrics import f1_score, precision_score, recall_score, fbeta_score, \
    ...
    label_ranking_average_precision_score, confusion_matrix, multilabel_confusion_matrix,mean_absolute_error
...
class PSNR:
    ...
    def forward(self, true, predicted):
        psnr = peak_signal_noise_ratio(true, predicted)
        return psnr
    ...
    @staticmethod
    def __call__(true, predicted):
        #psnr = PSNR(dynamic_range = data_ranges)
        #psnr.attach(result, "psnr")
        psnr = peak_signal_noise_ratio(true, predicted)
        return psnr
...
class SSIM:
    ...
    @staticmethod
    def __call__(true, predicted):
        ssim = structural_similarity(true, predicted)
        #ssim = SSIM(dynamic_range=data_ranges)
        #ssim.attach(result, "ssim")
        return ssim
...
class MAE:
    ...
    def __call__(true, predicted):
        ...
        #mse = mean_absolute_error(true[0:15,:,:], predicted[0:15,:,:])
        mse = np.mean(np.abs(true - predicted))
        #ssim = SSIM(dynamic_range=data_ranges)
        #ssim.attach(result, "ssim")
...
class SAM:
    ...
    def __call__(true, predicted):
        ...
        mat = np.multiply(true, predicted)
        mat = np.sum(mat, 1)
        mat = np.divide(mat, np.sqrt(np.sum(np.multiply(true, true), 1)))
        mat = np.divide(mat, np.sqrt(np.sum(np.multiply(predicted, predicted), 1)))
        sam = np.arccos(np.clip(mat, -1, 1))
